...
class ArticleEditPage extends Page {
    ...
    public function init(Website $oWebsite) {
        ...
        try {
            ...
            // Now check input
            if ($article_editor->process_input($_REQUEST, $this->categories_object)) {
                if (isset($_REQUEST["submit"])) {
                    // Try to save
                    $article = $article_editor->get_article();
                    if ($article->save($oWebsite->get_database())) {
                        if ($article_id == 0) {
                            // New article created
                        ...
                        $this->message.= ' <a class="arrow" href="' . $oWebsite->get_url_page("article", $article->id) . '">';
                        $this->message.= $oWebsite->t("articles.view") . "</a>";

                        // Check for redirect
                        if ($_REQUEST["submit"] == $oWebsite->t("editor.save_and_quit")) {
                            $this->redirect = $oWebsite->get_url_page("article", $article->id);
                        }
                    } else {
    ...
    public function get_page_title(Website $oWebsite) {
        ...
        if ($this->article_editor != null) {
            $article_title = $this->article_editor->get_article()->title;
            if ($article_title) {
    ...
    public function get_page_content(Website $oWebsite) {
        ...
        $show_comments = $article->show_comments ? 'checked="checked"' : '';
        $featured_image = htmlspecialchars($article->featured_image);
        if ($featured_image) {
        ...
        $date = "";
        $time = "";
        $date_time = explode(" ", $article->on_calendar);
        ...
        $return_value = <<<ARTICLE_FORM
            <!-- Featured image selector -->
            {$oEditor->get_image_selector_script("article_featured_image")}
            <!-- Date picker and CKFinder integration -->
            <script type="text/javascript" src="{$oWebsite->get_url_scripts()}article_editor.js"></script>
            <script type="text/javascript" src="{$oWebsite->get_sitevar("ckfinder_url")}ckfinder.js"></script>
            <script type="text/javascript">
                initialize("{$oWebsite->get_sitevar("ckfinder_url")}");
            </script>  
            ...
            <form action="{$oWebsite->get_url_main()}" method="post">
                ...
                <div id="sidebarpagecontent">
                    ...
                    <p>
                        ...
                        <a class="button" href="{$oWebsite->get_url_page("article", $article->id)}">{$oWebsite->t("editor.quit")}</a>
                    </p>
                </div>
...
ARTICLE_FORM;
    ...
    protected function get_category_list(Article $article) {
        ...
        $categories = $this->categories_object->get_categories();
...
$this->register_page(new ArticleEditPage());
?>