...
class ArticleListView extends View {
    ...
    public function getText() {
        ...
        if ($loggedInStaff) {
            $output.= '<p><a href="' . $oWebsite->getUrlPage("edit_article", 0, array("article_category" => $mainCategoryId)) . '" class="arrow">' . $oWebsite->t('articles.create') . '</a></p>';
        }
    ...
    public function getArticleTextSmall(Article $article, $show_metainfo,
        ...
        $returnValue.= '"onclick\"location.href=\'' . $oWebsite->getUrlPage("article", $article->id) . "'\" onmouseover=\"this.style.cursor='pointer'\">";
